...
/*!
 * @brief IFLRequest callback used for both success & failure callbacks
 * 
 * @var obj
 * An NSData object that contains the payload of the response
 *
 * @var resp
 * An NSURLResponse that holds the server response
 *
 * @var error
 * An NSError object that holds the error message generated by the request. Is nil if no error occured
 */
typedef void (^IFLCallBack)(id obj, NSURLResponse* resp, NSError* error);


/*!
 * @brief IFLRequest callback that informs the block of the download status
 *
 * @var session
 * A reference to the current NSURLSession that is managing the reqeust
 *
 * @var downloadTask
 * A reference to the current download task. All IFLRequest generate a download task regardless if its download JSON data or image data
 *
 * @var byesWritten
 * The number of bytes written to for this call
 *
 * @var totalBytesWritten
 * The total number of bytes written. Since this method gets called mutliple times throughout the download this variable has the summation of the total bytes
 *
 * @var totalExpectedBytesToWrite
 * The number of bytes the session expects for this download request
 */
